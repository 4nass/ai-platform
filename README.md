...
**How a run works:**
1. `core/context/manager.py` indexes the repo (tree-sitter chunking for Python, section chunking for Markdown, whole-file otherwise) into a local, file-mode Qdrant vector index under `.ai-platform/`, then selects the chunks/files relevant to the request, plus the target's own `memory/*.md`. During a run it reads from the run's **integration worktree** — a checkout of the base commit — not from your working tree, so the prompt describes exactly the tree the agents are editing; the index itself stays under `target_root`, which outlives the run.
...
A dirty target working tree no longer blocks a run — nothing writes to it, and nothing reads from it either. A run works on the base commit alone: tracked modifications, untracked files and uncommitted `memory/*.md` edits are all outside it, and the run says how many it left out (`4 local modifications excluded from the run`) rather than warning that your tree is dirty, which you already know. `--dirty-policy=reject` refuses to start instead; `--dirty-policy=snapshot` — reproducing the dirty state inside the run — is declared but not implemented, and says so rather than quietly falling back. The run lock is taken before the first mutating operation, so a run refused because another is in progress leaves no branch and no worktree behind. A stage that raises unexpectedly — a role missing from `config/agents.yaml`, a provider throwing rather than returning a failure — fails only itself: the DAG degrades around it and its worktree is still reclaimed, rather than the exception killing every other in-flight stage and stranding the directory (issue #1).
