...
reusing it with changed inputs is rejected. A failed action is never retried
implicitly.
...
handler through cancel_event. A running external call must implement its own
safe cancellation; the executor records cancel_requested and never starts a
second call automatically.

The action database is in jobs.sqlite with the job coordination data. Backups
and retention must treat action audit events as security-sensitive records.
